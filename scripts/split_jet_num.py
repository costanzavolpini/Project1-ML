...
def reasseambly_tx_labels_ids(tx_old, tx0, tx1, tx2, tx3, ids0, ids1, ids2, ids3, w0, w1, w2, w3):
	...
	print(rows_0_indexes)
	# return True
	print(rows_0_indexes[0])
	final_w = np.zeros(tx_old.shape[0])
	# ciao[np.array([3, 4, 5])] = np.array([1000, 3, 6])
	print(rows_0_indexes[0].shape)
	print(w0.shape)
	final_w[np.array(rows_0_indexes[0])] = np.array(w0)
	final_w[np.array(rows_1_indexes[0])] = np.array(w1)
	final_w[np.array(rows_2_indexes[0])] = np.array(w2)
	final_w[np.array(rows_3_indexes[0])] = np.array(w3)


	print(final_w)

	# joinate = np.concatenate((jet_num_0, jet_num_1,jet_num_2, jet_num_3), axis=0)
	# print("lll", joinate)
